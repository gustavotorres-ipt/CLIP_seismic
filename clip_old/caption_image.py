...
from dataset import IMAGE_FOLDER_VALIDATION, TEXT_FOLDER_VALIDATION, read_captions_json
import os
import random
import numpy as np
import torch
...
def load_captions():
    captions_files = [os.path.join(TEXT_FOLDER_VALIDATION, f)
                      for f in os.listdir(TEXT_FOLDER_VALIDATION)]
...
def load_image():
    selected_image = random.choice(os.listdir(IMAGE_FOLDER_VALIDATION))
    img_path = os.path.join(IMAGE_FOLDER_VALIDATION, selected_image)
